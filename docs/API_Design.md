...
元数据质量接口不会修改源端分类，也不会覆盖 `metadata_table.business_domain`。当前将空业务域、`未分类` 和 `好股库未分类` 统一视为“待治理分类”，用于识别后续需要人工补充或推荐分类的表。质量分是服务端根据下线状态、分类、表中文名、字段数、主键、字段中文名、字段备注、是否为空和字段类型异常计算的治理优先级，不代表源端数据真实质量评级。未分类诊断会结合表名命名模式、血缘引用次数、表中文名和主键信息，拆分为“仍被血缘引用”“疑似测试或临时表”“疑似遗留无用表”和“待补业务分类”。`metadata_table.updated_at` 表示本项目本地元数据记录更新时间，不能直接代表公司业务表真实数据更新时间，因此当前不把它作为报废判断的唯一依据。
